...
**Открытые вопросы владельцу:** нет — все закрыты.

РЕШЕНО владельцем (2026-09-14):
- (1) debounce записи phrases_status — НЕТ, писать сразу;
- (2) staleness UI — иконка в строке + подсветка поля RU (рендеринг
  M4/M5, данные — M2.5);
- (3) дубли текста с разными guid — НЕЗАВИСИМЫЕ элементы;
- (4) автодетект 2/3 уровней вложенности — агрегат sample.json
  остаётся читаемым (зафиксировано в roadmap M2.5 п.1 и
  architecture §2); п.9-бис §M3 переименован в «M3-followup:
  автофокус в поле RU».
