...
### Sidecar-контракт (задача владельца 2026-09-14, вариант B — только docs)

**Сделано (код Sidecar НЕ писался, M2.5 спроектирован):**
- Ревизия M1-парсера (отчёт «Факт vs требуемое» a-f — в отчёте задачи):
  парсер ждёт агрегат 3 уровней (file_id->quest_id->guid,
  dubbingjsonreader.cpp:303-311), реальный формат студии — ДВА уровня
  (файл = quest-файл, scene_id->guid); dur обязателен (пропуск реплики
  без dur, :323-327); источника файла у Line нет; setLineRu — только
  in-memory домен + .aup4; склейки по тексту нет (дедуп только по guid
  в сцене, :342-347); по scene_id как по спикеру НЕ группирует (спикер
  — из полей записи).
- architecture.md: примечание о реальном формате в §2; новые §16
  (Sidecar-контракт) и §17 (пакет MyDub + WORK-зона M4).
- roadmap.md: mermaid + новый §M2.5 (чеклист) перед M4; пометка в §M3
  «RU пишет только in-memory -> phrases_status в M2.5»; контракт
  WORK-зоны в §M4.
- Автофокус RU (п.7a задачи) уже в ветке (коммит 07059e21a).
- Тесты M2/M3 НЕ менялись: dubbing_tests 19/19, полный ctest 29/29.

**Открытые вопросы владельцу:** (1) debounce записи phrases_status
(рекомендация: писать сразу, debounce по замерам); (2) вид UI при
staleness override (base_ru_snapshot != новый ru); (3) подтверждение
трактовки дублей текста с разными guid (в т.ч. без dur) как независимых.
РЕШЕНО владельцем (2026-09-14, доработка до пуша): (4) автодетект
2/3 уровней вложенности — агрегат sample.json остаётся читаемым
(зафиксировано в roadmap M2.5 п.1 и architecture §2); п.9-бис §M3
переименован в «M3-followup: автофокус в поле RU».

**Пуш и мерж (по разрешению владельца 2026-09-14):** ветка
feature/dubbing-m3-panel запушена (origin), master обновлён ff
9fd6c4dfd -> cdb8fb651 и запушен (31 файл, +3206/-5). M3 закрыт
полностью; следующий шаг — M2.5 Sidecar (жду промт владельца; открытые
вопросы: debounce, UI staleness, подтверждение независимости
текстов-дублей).
