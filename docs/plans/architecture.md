...
Решение владельца (2026-09-14): debounce НЕТ — писать сразу (файл
маленький, per-quest, ~сотни записей), undo перезаписывает файл целиком
из снапшота команды. Пересмотр — только при замере реальных нагрузок.
...
### 16.8 Сценарий «Новый проект из quest-файлов» (формирование из пустого)

Критерий готовности M2.5; сейчас нигде не закодирован — контракт.

**Первичное формирование.** Пользователь создаёт пустой дубляж-проект
и указывает: quest-JSON (файл или папка — автодетект 2/3 уровней,
см. §16.2/решение о совместимости) и папку с WAV-референсами
({guid}.wav). Приложение:

1. копирует исходные quest-файлы в `phrases/{quest}.json` — read-only
   зеркало (байт-в-байт; исходники студии после копирования не
   читаются и не пишутся);
2. копирует сопоставленные референсы в `refs/{guid}.wav` (плоский
   guid-namespace, §17); отсутствующий референс не блокирует
   остальные (статус «нет референса»);
3. создаёт пустой `phrases_status/` рядом (по одному файлу на quest —
   создаются лениво при первой записи, либо пустые сразу);
4. строит домен: GameFile.fileId = имя quest-файла (без пути, без
   расширения), сцены/реплики по §16.2, референсные клипы — на
   скрытой REF-дорожке (механика M2);
5. .aup4 сохраняется в корне пакета MyDub/ (§17).

Явно: один дубляж-проект = один файл игры (решение принято, §14), но
домен допускает несколько GameFile — при выборе папки с несколькими
quest-файлами поведение: домен принимает все, Select файла M3
переключает; рекомендация рабочему процессу — один файл игры на проект.

**Повторный импорт новой версии phrases/{quest}.json** (геймдиз прислал
обновление): пользователь указывает новый исходник -> приложение
обновляет зеркало phrases/ и домен: ru/en/speakers/dur берутся из новой
версии; `ru_override` в phrases_status/ НЕ затирается; для каждого guid
с override сверяется `base_ru_snapshot` с новым ru — расхождение
фиксируется как ДАННЫЕ для будущего staleness-UI (warning «перевод
обновлён студией, проверь свою правку» — рендеринг в M4/M5); новые
guid добавляются, исчезнувшие помечаются (не удаляются — инкрементальность
M2). Одним undo-шагом на пакет (как importProject M2).
